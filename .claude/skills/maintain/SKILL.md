...
Run `/close` to finalize the session log (debrief capture, State at Close, status),
check for loose ends (uncommitted changes, documentation mirrors, validation), and
summarize state for next time.
