...
- **Prerequisites** — Framework at `packages/nla-framework/`, Claude Code installed.
...
maintained by `/install` and `/update`. Contains shell command approvals (`Bash(git:*)`,
`Bash(ls:*)`, `Bash(test:*)`).

Gitignored by Claude Code automatically — permissions are machine-specific. Not every
