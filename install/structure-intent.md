...
├── .claude/settings.local.json   # Claude Code permissions (gitignored automatically)
...
Claude Code permission configuration for this project. Generated by `/create-app` and
maintained by `/install` and `/update`. Contains filesystem access patterns — read
permissions for the framework and packages, common bash command approvals.

Gitignored by Claude Code automatically — permissions are machine-specific (absolute
paths vary by installation). Not every entry needs to be in this file; Claude Code
prompts for permission when access isn't pre-approved. The settings file eliminates
prompts for routine operations (reading framework files, running git commands).
