...
`/write-letter`) exist; you don't need to replicate their work. If the session is
wrapping up, suggest `/close` to finalize the session record — debrief observations
land in the session log's Debrief section.
